...
class Loader:
    ...
    def clean(self, df):
        ...
        dfx = df.groupby(by=['meter_id', 'meter_type'])[['address']].count().sort_values(
    ...
    def _df(self):
        ...
        df_lg = self.extract(
            df, monitor_types=['lightgauge', 'egauge'], monitor_regex=r'^(lg)?(\d+)$', meter_tag='lightgauge')
        df_en = self.extract(
            df, monitor_types=['enphase envoy wifi'], monitor_regex=r'([^\d]*)(\d+)$', meter_tag='enphase')
